...
int minimumMinutia = 35; //default minimum

int silentMinimum=10;//To accept this as a minimum when several tries have been made
...
//ui->lblRating->setText("");
        double tPercent;
        double fPercent;
        int qPercent = 50 - ((fingerPrintQuality-1) * 10);

        if(minutiaCount<minimumMinutia){

             fPercent = (minutiaCount/minimumMinutia) * 50;
         ...
         tPercent = (qPercent+fPercent)*0.6;//to force it down
         QString qLog;
         qLog.sprintf("%s\% [%d, %d]",QString::number(tPercent).toStdString().c_str(),minutiaCount,fingerPrintQuality);
         ui->lblRating->setText(qLog);


        if( minutiaCount < minimumMinutia || fingerPrintQuality > 3) {
...
//                    if( failedStats[fFingerScan][0] > trueBestMinutia &&  (
//                            failedStats[fFingerScan][2]<=2 || (
//                                        failedStats[fFingerScan][2]<=3 && // fingerQuality is at least 3
//                                        ( (( printIndex == 1 || printIndex==2 || printIndex==6 || printIndex==7 ) && failedStats[fFingerScan][0] > 59)
//                                                ||
//                                                (printIndex == 3 || printIndex==4 || printIndex==5 || printIndex == 8 || printIndex==9 || printIndex==10))// and finger is either 3,4,5 or 8,9,10 -last three fingers on each hand
//                                )
//                            )
...
//                           ) {
                    ...
                    if(failedStats[fFingerScan][0] > trueBestMinutia && fingerPrintQuality <=5){ // fingerQuality is at least level 4
        ...
        int currentPrintInd = printIndex;
        ...
        emit(initiateBozorthCheck(newEnrollFile, currentPrintInd,minutiaCount,minimumMinutia,fingerPrintQuality));
        ...
        case -105:
            ...
            break;
        case 0:
            log = "- - - Please click Restart Device - - -";
            displayMessage(log,0);
            deviceError  = true;
            break;
        case -22:
...
int FingerDialog::isDoubleScanned(int printIndex,QString newEnrollFile){
  ...
  */
    ...
    if(printIndex > 1) {
        ...
        QString shPassword = "JwgVN5aHth8J7";
        fullBozorth3Exe.sprintf("scripts/duplicate_scan.sh enrolled_binarized.pgm %d %d %d %s",printIndex,bzMatchThreshold,bzMatchMinutiae,shPassword.toUtf8().data());
