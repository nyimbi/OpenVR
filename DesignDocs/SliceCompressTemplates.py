#!/usr/bin/python
#This picks templates in a dir, slices  and compresses the first and second fingers onboth hands
...
#        print("Compressing to {0}".format(dstFile))
        sourceContentsLines = readDataFile(srcFile, True)
        stackedLines = []
        totalLines = len(sourceContentsLines)
        
        linesRead=0    
        for eachLine in sourceContentsLines:
                    
                stackedLines.append(eachLine)
                linesRead=linesRead+1
                if linesRead>maxMinutiaRequired:
                    break
                    
        sourceContents  = "".join(stackedLines);
        try:
            cfile = open("{0}.sliced".format(dstFile), 'w')
            cfile.write(sourceContents)
            cfile.close()
        except IOError as err:
            logError ("WriteError:{0}", "{0}.sliced".format(dstFile))    
