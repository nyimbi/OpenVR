...
MOUNT_ENC_SCRIPT="mount_disc.sh"
UNMOUNT_ENC_SCRIPT="unmount_disc.sh"
...
while read curdisk; do
	...
	ls "/tmp$curdisk/$EXT_BK_DIR" |
	while read macDir; do
		echo $macDir
		bkfile=`ls /tmp$curdisk/$EXT_BK_DIR/$macDir/database/$DB_BACKUP_EXTENSION -t | head -n 1 `
		importfile "$bkfile"
		rm "$bkfile"
	done

	echo "$PASSWORD" | sudo -S rm -rf "/tmp/$curdisk"/*
done
