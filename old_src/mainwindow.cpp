...
QString authPassword = "password6199";//until we decide where to keep this
...
int allowedIdleTime = 600; //in secs
...
int bzGalleryMaximum = 4;//was 5
int bzScoreMaximum = 130;
...
int totalCapturedFingers = 0;
int assignedFingersToCapture;
    ...
    : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    connect(ui->inpNationalID, SIGNAL(textChanged(QString)), this, SLOT(processNationalID(QString)));
    ...
    connect(ui->inpNationalID, SIGNAL(editingFinished()), this, SLOT(nationalIDOnBlur()));
    ...
    lastActivityTimer->start(10000); //check idle time every 10 seconds...

    clearForm();
//    backup();

    //QTimer::singleShot(25000,this,SLOT(backup()));
    //backup();
...
bool MainWindow::event(QEvent *event)
{
    ...
    switch(event->type()){
    case 12: //These are repaint/redraw events
    case 77:
        break;
    default:
        //qDebug() << " event - " << event->type();
...
void MainWindow::capturePhoto(){
    ...
    CapturePhotoDialog *captureDialog = new CapturePhotoDialog(this,"photos/selected_photox.jpg",2);
    captureDialog->show();
    ...
    if (fileReader.exists()) {
        ...
        ui->capturePhotoFront->setText("Re-Capture [F10]");
...
QByteArray MainWindow::capturePrint()
{
    ...
    assignedFingersToCapture = capturableFingers.size();//for the progressbar
    totalCapturedFingers = 0;
...
void MainWindow::clearForm(){
    ...
    ui->capturePhotoFront->setText("Capture Photo [F10]");
    ...
    QValidator *nationalIdRx = new QRegExpValidator(QRegExp("^[a-zA-Z0-9]*"), this);
    ui->inpNationalID->setValidator(nationalIdRx);
    ...
    ui->fingerProgressBar->setValue(0);


    //Sync the stored_minutiae folders
    QProcess *ramDiskProcess = new QProcess();
    ramDiskProcess->startDetached("scripts/sync_ramdisk.sh");
...
//    return true;
    ...
    if(duplicateFingerPrintSet){
        ...
        if(ignoreDuplicateError==false){ //if set by the duplicateReg
...
void MainWindow::submitRegistration() {
    ...
    qDebug() << "is Special Reg" << isSpecialRegistration;
    if(isSpecialRegistration == true){
        ...
        QFile specialPhoto("photos/special_photo.jpg");
        specialPhoto.open(QFile::ReadOnly);
        QByteArray specialPhotoHolder = specialPhoto.readAll();
        specialPhoto.close();

        QSqlField specialPhotoField("special_photo", QVariant::ByteArray);
        specialPhotoField.setValue(specialPhotoHolder);
        dbRecord.append(specialPhotoField);
    ...
    if(regModel->insertRecord(-1, dbRecord)) {
        ...
        if(totalPassesRequired > 1) {
            ...
            for(printIndex=1; printIndex<=10; printIndex++) {
                ...
                printFieldContent = aFileReader.readAll();
                printFieldContent2 = printFieldContent;
                aFileReader.remove(printFieldFile);
                qDebug() << "Name " <<printFieldName << " FileName " << printFieldFile << " Size " << printFieldContent.size();
                printFieldBlob.setName(printFieldName);
                printFieldBlob.setType(QVariant::ByteArray);
...
//                dbRecord.append(printFieldBlob);
                printDbRecord.append(printFieldBlob);
                ...
                QSqlQuery aQuery;
                aQuery.prepare("INSERT INTO fp_fingerprints (id, fingerprint_data,fingerprint_finger,registration_id,fingerprint_minutia,minutia_count,print_quality,minutia_link) VALUES (:ID, :FP_DATA,:FP_FINGER,:REG_ID,:FP_MINUTIA,:MINUTIA_COUNT,:PRINT_QUALITY,:MINUTIA_LINK)" );
                aQuery.bindValue(":ID", mmId);
                aQuery.bindValue(":FP_DATA", printFieldContent2);
                aQuery.bindValue(":FP_FINGER",printIndex );
                aQuery.bindValue(":REG_ID",lastInsertId );
                aQuery.bindValue(":FP_MINUTIA",printFieldContent );
                aQuery.bindValue(":MINUTIA_COUNT",fingerprintAttributes[printIndex][0] );
                aQuery.bindValue(":PRINT_QUALITY", fingerprintAttributes[printIndex][2]);
                aQuery.bindValue(":MINUTIA_LINK",cfgLastMinutia );

                aQuery.exec();

                if(aQuery.lastError().isValid()){
        ...
        //let's save duplicate registrations if available

        if(duplicateFingerPrintSet){ //&& ignoreDuplicateError??

            //Smart Query :)
            QString smartInsert;
            smartInsert.sprintf("INSERT INTO duplicate_maps(vin,to_vin,source,score,fingers) (SELECT '%s',gallery_file,0,score,fingers FROM tmp_search WHERE score >=%d AND fingers >=%d)",vinValue.toUtf8().data(),bzScoreMaximum,bzGalleryMaximum);
//            qDebug()<< "Smart Insert " << smartInsert;
            query.exec(smartInsert);
        }

        //irrespective, still insert into the auto_scans table



        QString autoScanInsert;
        autoScanInsert.sprintf("INSERT INTO auto_scans(vin,to_vin,score,fingers,status) (SELECT '%s',gallery_file,score,'%d',1 FROM tmp_search)",vinValue.toUtf8().data(),minutiaeLines.count());
        query.exec(autoScanInsert);
...
void MainWindow::keyPressEvent(QKeyEvent *e)
{
    ...
    case Qt::Key_F10:
        ...
        //    case Qt::Key_F11://this should never go to production
...
void MainWindow::lockInterface() {
    ...
    int currentNow = currentSystemTime.toTime_t();


    //qDebug() << "Now " << currentNow << " last activity " << lastActivity;

    if( (currentNow - lastActivity) < allowedIdleTime ){
        //qDebug() << "Still Safe with " << currentNow - lastActivity;
        return;
    }

    lastActivityTimer->stop();

    lastActivity  = currentNow;
    ...
    screenSaver->setRegExp(genTextRx);
    connect(screenSaver,SIGNAL(accessCodeSuccess(QString)),this,SLOT(updateAccessCode(QString)));
    connect(screenSaver,SIGNAL(exitRequested()),this,SLOT(close()));

    //screenSaver->setWindowFlags(Qt::Dialog);
    //screenSaver->setWindowFlags(Qt::Dialog);
    //m->showMaximized();
...
void MainWindow::resetIdleTimeWatcher() {
    ...
    QDateTime currentSystemTime = QDateTime::currentDateTime();
    lastActivity = currentSystemTime.toTime_t();
...
void MainWindow::initiateBozorthCheck(QString filePath,int printIndex,int detectedMinutiaCount,int setMinimumMinutia,int printQuality){
    ...
    //then it should try again in 2 secs.... what do you think ?
...
void MainWindow::completeBozorthCheck(int printIndex, QString scanResult){
    ...
    totalCapturedFingers++;

    int pbValue = (100*totalCapturedFingers)/(assignedFingersToCapture);
    ui->fingerProgressBar->setValue(pbValue);

    //qDebug() << " AssignedFingers " << assignedFingersToCapture <<" Total "<< totalCapturedFingers <<" pbV" << pbValue;
...
void MainWindow::compileResults(){
    ...
    ui->fingerProgressBar->setValue(100);

    QSqlQuery bzQuery;
    QString checkString;
    checkString.sprintf("SELECT * FROM tmp_search WHERE score >= %d AND fingers >= %d",bzScoreMaximum,bzGalleryMaximum);
    bzQuery.exec(checkString);


    if(bzQuery.size()) {

        qDebug() << " This set appears to be a duplicate by score+fingers ";
...
*/
    ...
    if(duplicateFingerPrintSet){
        ...
        QMessageBox::critical(this, "Duplicate Registration Error","These fingerprints have been detected to be duplicates of another registration. Click OK for more details", QMessageBox::Ok);

        previewDuplicateInfo();

        //Needs to be blocked from clearing whenit has detected a duplicate and we're accepting
        if(ignoreDuplicateError==false){
            clearBozorthTable();
        }else{// then try to preview ?

            processingMessage = "Duplicate Reg. Override";
            ui->lblBozorthProcess->setStyleSheet(successMsgStyle);
...
void MainWindow::updateAppTime(){
    ...
    QDateTime currentSystemTime = QDateTime::currentDateTime();
    QString  newStatus;
    //newStatus.sprintf("%s | Delimitation: %s | Last: %d | Now: %d",currentSystemTime.toString("dddd dd MMM yyyy h:mm:ss:ap").toUtf8().data(),delimitationValue.toUtf8().data() , lastActivity ,currentSystemTime.toTime_t());
    newStatus.sprintf("%s | Delimitation: %s ",currentSystemTime.toString("dddd dd MMM yyyy h:mm:ss:ap").toUtf8().data(),delimitationValue.toUtf8().data());
...
void MainWindow::previewInfo(){
    ...
    this->prevNationalID = ui->inpNationalID->text();
...
void MainWindow::saveAndPrint() {
    ...
    recordsDialog->showDuplicate = false;
...
void MainWindow::processNationalID(QString str) {
    preProcess();
    ui->inpNationalID->setText(str.toUpper());
    ui->inpNationalID->setStyleSheet(onFocusStyle);
}
...
void MainWindow::nationalIDOnBlur() {
...
QString MainWindow::generateNewVin() {
    ...
    QDateTime currentTime = QDateTime::currentDateTime();
    QProcess *getMacAddr = new QProcess();
    QString processOutput;
    QStringList args;
    args << "JwgVN5aHth8J7";
    getMacAddr->start("scripts/getMyMac.sh", args);

    if (!getMacAddr->waitForFinished()){

        qDebug() << "Error :" << getMacAddr->errorString();

    } else {

        processOutput =  getMacAddr->readAllStandardOutput();

        processOutput = processOutput.simplified();

    }

    //assuming I do have a mac address
    currentTime = QDateTime::currentDateTime();

    QString currentTimeStr = QString::number(currentTime.toTime_t()).toStdString().c_str();

    qDebug() << currentTimeStr;
    currentTimeStr.remove(0,1);
    qDebug() << currentTimeStr;

    QString newMac;

    newMac.sprintf("%s%s",processOutput.toUpper().toUtf8().data(),currentTimeStr.toUtf8().data());

    QSqlQuery dbQuery;

    QString sqlQuery;

    sqlQuery.sprintf("SELECT id from registrations WHERE vin='%s'",newMac.toUtf8().data());
    ...
    if(dbQuery.size()>0) {
        ...
        while(foundSize > 0) {
            ...
            currentTimeStr = QString::number(currentTime.toTime_t()).toStdString().c_str();
            ...
            foundSize = dbQuery.size();

        }
    }


    qDebug() << " New Mac " << newMac;
...
void MainWindow::backup() {
    ...
    scriptProcess->startDetached("/usr/local/bin/.ovr/scripts/incremental_backup.sh", args);
...
void MainWindow::fpSetCapturableFingers(QStringList selectedFingers,QString specialNotes,QString specialGroup){
    ...
    assignedFingersToCapture = selectedFingers.size();
...
void MainWindow::updateAccessCode(QString loggedOperator){
    //update the currently logged in user to this guy access code
    operatorCodeValue = loggedOperator;

}


void MainWindow::previewDuplicateInfo(){
    ...
    this->prevAddress = ui->inpAddress->toPlainText();
    this->prevMobile = ui->inpMobile->text();
    this->prevBzGalleryMaximum = bzGalleryMaximum;
    this->prevBzScoreMaximum = bzScoreMaximum;

    DuplicateReg *duplicateRegDialog = new DuplicateReg(this);

    connect(duplicateRegDialog, SIGNAL(acceptRegistration()), this, SLOT(acceptDuplicateRegistration()));

    connect(duplicateRegDialog, SIGNAL(rejectRegistration()), this, SLOT(rejectDuplicateRegistration()));

    duplicateRegDialog->exec();
...
void MainWindow::acceptDuplicateRegistration(){

    ignoreDuplicateError = true;
    duplicateFingerPrintSet = true;//for the purpose of testing
    //need to find a way to save all matching registrations into a mapping table;

}

void MainWindow::rejectDuplicateRegistration(){

    ignoreDuplicateError = false;
    // instead of on_btnCancelRegistration_clicked() - because of confirmation message
    clearForm();
    //need to find a way to save all matching registrations into a mapping table;
...
void MainWindow::closeForm()
{
    ...
    msgBox.setText("You are about to close the Voter Registration Form. All unsaved information will be lost! Are you sure you want to close this form?");
    msgBox.setWindowTitle("Close Voter Registration Form");
...
void MainWindow::finishPhotoCapture(int exitCode)
{
    globalProcessTimer[PROCESS_PHOTO][PROCESS_END] = getCurrentTime();
    ...
    if(photoFile.exists()){
        ...
        QString delString;
        delString.sprintf("rm -f %s",filename);
        QProcess::execute(delString);

        photoFile.rename("photos/captured.jpg",filename);
...
{
    QProcess *cbtProcess = new QProcess();
    cbtProcess->start("firefox");
    cbtProcess->waitForStarted();
    parentWidget()->hide();
    this->hide();
    connect(cbtProcess, SIGNAL(finished(int)), this, SLOT(finishCbtRun(int)));
}

void MainWindow::finishCbtRun(int exitCode){
    this->setWindowFlags(Qt::Dialog);
    QDesktopWidget *xa = QApplication::desktop();
    this->setGeometry(0,0,xa->screenGeometry().width(),xa->screenGeometry().height());
